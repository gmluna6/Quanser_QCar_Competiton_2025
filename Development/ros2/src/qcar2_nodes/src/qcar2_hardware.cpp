...
public:
    ...
    : Node("qcar2")
    {
        ...
        t_error led_result =  aaaf5050_mc_k12_open(LED_uri_param.c_str(), 33, &led_strip);
        msg_get_error_messageA(NULL, led_result, error_message, sizeof(error_message));
...
private:
    ...
    void LED_Set(){
        ...
        while ( delta_time.seconds() < 0.2)
        {
            ...
            t_int result = aaaf5050_mc_k12_write(led_strip, color, 33);
            // RCLCPP_ERROR(this->get_logger(), "Error code when writing to LEDs: %i", result);
        }
